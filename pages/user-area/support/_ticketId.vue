            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="ticket">
                        ...
                        <hr>
                        <p class="c-white">{{selectedTicket.replies.length}} replies</p>
                        <p class="c-white"><strong>Created on:</strong> {{ new Date(selectedTicket.created).toDateString() }}</p>
                        <span v-if="selectedTicket.status==='NEW'" class="badge badge-pill badge-warning float-right">{{ selectedTicket.status }}</span>
                        <span v-if="selectedTicket.status==='IN_PROGRESS'" class="badge badge-pill badge-primary float-right">{{ selectedTicket.status }}</span>
                        <span v-if="selectedTicket.status==='CLOSED'" class="badge badge-pill badge-success float-right">{{ selectedTicket.status }}</span>
                    </div>
                </div>
                ...
                <div v-if="selectedTicket.replies.length === 0" class="col-md-12 text-center">
                    <p class="c-white meta">No responses yet to this ticket. Click on "Respond" above to leave a response for the admin now</p>
                </div>
                ...
                >
                    ...
                    <p v-if="userDetails.username !== response.creator.username" class="warn">From Admin</p>
                    <p class="c-white">
                        {{ response.content }}
                    </p>
                    <p class="c-white meta">{{new Date(response.created).toDateString()}}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="container ml-3">
              <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <textarea v-model="response" placeholder="What is the complaint?"></textarea>
